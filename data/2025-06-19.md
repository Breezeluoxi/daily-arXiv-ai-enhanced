...
### [1] [Combining Constrained and Unconstrained Decoding via Boosting: BoostCD and Its Application to Information Extraction](https://arxiv.org/abs/2506.14901)
*Marija Šakota,Robert West*

Main category: cs.CL

TL;DR: BoostCD方法通过结合约束和非约束解码，分两阶段生成高质量结构化输出，解决了传统方法在测试时解码质量低的问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法在训练时不考虑约束，导致测试时约束解码输出质量低。BoostCD旨在通过结合约束和非约束解码提升性能。

Method: BoostCD分两阶段：第一阶段分别进行约束和非约束解码生成两个弱预测；第二阶段通过增强模型结合两者生成最终预测。

Result: BoostIE在封闭信息提取任务中表现优于现有方法，分布内外均有效。

Conclusion: BoostCD通过互补性错误利用，显著提升了结构化NLP任务的解码质量。
...
Conclusion: CrEst能有效提升LLM在噪声环境下的性能，且无需人工标注。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏针对多文档推理的评估基准，而多文档推理对大型语言模型的能力测试至关重要。

Method: 采用基于结构化种子知识的合成生成技术，通过LLM辅助编辑生成文档集和对应的问答示例。

Result: MDBench对现有LLM和提示技术提出了显著挑战，且生成方法支持快速适应新挑战。

Conclusion: MDBench填补了多文档推理评估的空白，其生成方法高效且可扩展。
...
### [4] [From Chat to Checkup: Can Large Language Models Assist in Diabetes Prediction?](https://arxiv.org/abs/2506.14949)
...
### [5] [Memory Tokens: Large Language Models Can Generate Reversible Sentence Embeddings](https://arxiv.org/abs/2506.15001)
*Ignacio Sastre,Aiala Rosá*

Main category: cs.CL

TL;DR: 研究发现，通过引入特殊记忆令牌，可以生成可逆句子嵌入，使LLM无需修改权重即可精确重建原始文本。


<details>
  <summary>Details</summary>
Motivation: 探索LLM在无需修改权重的情况下精确重建文本的能力及其潜在应用。

Method: 通过训练优化特殊记忆令牌的嵌入，使其能提示模型重建固定序列。

Result: 在英语和西班牙语数据集、最长约240个令牌的序列及100M到8B参数的模型规模下均成功实现重建。

Conclusion: 该发现展示了LLM的有趣能力，并可能在记忆检索、压缩和受控文本生成中有应用价值。
...
### [6] [Identifying social isolation themes in NVDRS text narratives using topic modeling and text-classification methods](https://arxiv.org/abs/2506.15030)
*Drew Walker,Swati Rajwal,Sudeshna Das,Snigdha Peddireddy,Abeed Sarker*

Main category: cs.CL

TL;DR: 该研究利用自然语言处理技术分析执法和法医叙述，识别社会孤立和孤独感，开发高质量分类器，并发现特定人群（如男性、同性恋、离婚者）更容易被分类为慢性社会孤立。
...
Method: 使用主题建模生成词典，结合监督学习分类器，分析30万例自杀案例（2002-2020年）。

Result: 分类器表现优异（平均F1：0.86，准确率：0.82），识别出1,198例慢性社会孤立案例，特定人群风险更高。

Conclusion: 该方法可改善美国社会孤立和孤独感的监测与预防。
...
### [7] [Semantically-Aware Rewards for Open-Ended R1 Training in Free-Form Generation](https://arxiv.org/abs/2506.15068)
...
### [8] [Learning-Time Encoding Shapes Unlearning in LLMs](https://arxiv.org/abs/2506.15076)
*Ruihan Wu,Konstantin Garov,Kamalika Chaudhuri*

Main category: cs.CL

TL;DR: 论文研究了学习阶段知识编码对事后遗忘（unlearning）效果的影响，发现使用转述描述学习可提升遗忘性能，但遗忘文本中的单个知识点仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLMs）的广泛应用，事后遗忘特定知识的能力变得至关重要，涉及隐私法规和纠正有害内容等问题。

Method: 通过实验研究学习阶段知识编码对遗忘效果的影响，重点关注转述描述学习和文本块中单个知识点的遗忘。

Result: 发现转述描述学习可提升遗忘性能，但遗忘文本中的单个知识点较困难。
...
### [9] [Improving Dialogue Discourse Parsing through Discourse-aware Utterance Clarification](https://arxiv.org/abs/2506.15081)
...
<details>
  <summary>Details</summary>
Motivation: 对话中的语言特征（如省略和习语）常导致话语关系模糊，增加了解析难度，需要一种新方法来处理这些歧义。

Method: 提出DCM模块，包含澄清类型推理和话语目标推理；引入CPO优化DCM的输出，减少错误澄清的级联影响。

Result: 在STAC和Molweni数据集上的实验表明，该方法有效解决歧义，性能显著优于现有基线。

Conclusion: DCM和CPO的结合显著提升了对话话语解析的准确性和鲁棒性。
...
### [10] [CKD-EHR:Clinical Knowledge Distillation for Electronic Health Records](https://arxiv.org/abs/2506.15118)
...
<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在医学知识表示和临床部署效率上存在不足，需改进以支持精准医疗和早期干预。
...
### [11] [Modeling the One-to-Many Property in Open-Domain Dialogue with LLMs](https://arxiv.org/abs/2506.15131)
*Jing Yang Lee,Kong-Aik Lee,Woon-Seng Gan*

Main category: cs.CL

TL;DR: 论文提出了一种两阶段框架（MRG和PS）来显式建模开放域对话的一对多属性，通过新数据集o2mDial和评估方法提升小模型的表现。
...
Method: 将对话生成分解为多响应生成（MRG）和基于偏好的选择（PS），并引入o2mDial数据集及新学习策略与评估指标。

Result: 实验表明，该框架显著提升小模型的响应多样性和质量（最高90%），接近大模型表现。

Conclusion: 显式建模一对多属性可有效提升对话生成质量，小模型通过两阶段框架能实现更优性能。
...
### [12] [Thunder-Tok: Minimizing Tokens per Word in Tokenizing Korean Texts for Generative Language Models](https://arxiv.org/abs/2506.15138)
...
<details>
  <summary>Details</summary>
Motivation: 设计一个更高效的韩语分词器，减少分词数量以提高推理速度。

Method: 结合基于规则的分词方法和分支熵选择算法，构建种子词汇表以保留语言信息。

Result: 实验显示，Thunder-Tok比BPE分词器减少约10%的分词数量，推理速度提升10%，且性能不受影响。
...
### [13] [Emergence of Primacy and Recency Effect in Mamba: A Mechanistic Point of View](https://arxiv.org/abs/2506.15156)
*Muhammad Cendekia Airlangga,Hilal AlQuabeh,Munachiso S Nwadike,Kentaro Inui*

Main category: cs.CL

TL;DR: 研究通过首因和近因效应分析状态空间语言模型的记忆机制，发现Mamba架构在输入序列的首尾表现最佳，揭示了三种机制：稀疏通道支持长期记忆、delta调制递归控制短期记忆，以及语义规律动态调节记忆分配。
...
Method: 应用结构化回忆任务于Mamba架构，分析其U形准确率曲线，并通过目标消融和输入扰动验证。
...
### [14] [A Comparative Study of Task Adaptation Techniques of Large Language Models for Identifying Sustainable Development Goals](https://arxiv.org/abs/2506.15208)
*Andrea Cadeddu,Alessandro Chessa,Vincenzo De Leo,Gianni Fenu,Enrico Motta,Francesco Osborne,Diego Reforgiato Recupero,Angelo Salatino,Luca Secchi*

Main category: cs.CL

TL;DR: 论文研究了如何利用大型语言模型（LLMs）进行文本分类，以支持联合国可持续发展目标（SDGs）的进展跟踪。通过比较专有和开源LLMs，并评估零样本学习、少样本学习和微调的效果，发现优化后的小模型性能可媲美大模型。


<details>
  <summary>Details</summary>
Motivation: 跟踪SDGs进展因数据规模大且复杂而困难，文本分类模型和LLMs成为关键工具。

Method: 分析专有和开源LLMs在SDGs相关文本分类任务中的表现，评估零样本学习、少样本学习和微调的效果。

Result: 优化后的小模型性能与大型模型（如GPT）相当。

Conclusion: 提示工程优化的小模型在SDGs文本分类中具有潜力，可减少对大模型的依赖。
...
### [15] [ProtoReasoning: Prototypes as the Foundation for Generalizable Reasoning in LLMs](https://arxiv.org/abs/2506.15211)
*Feng He,Zijun Chen,Xinnian Liang,Tingting Ma,Yunqi Qiu,Shuangzhi Wu,Junchi Yan*

Main category: cs.CL

TL;DR: 论文提出ProtoReasoning框架，通过共享抽象推理原型提升大模型的跨领域推理能力，实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 探索大模型跨领域推理的机制，假设共享抽象推理原型是关键。

Method: 提出ProtoReasoning框架，包括原型构建、验证系统和可扩展性设计。

Result: 在逻辑推理、规划任务、通用推理和数学任务上分别提升4.7%、6.3%、4.0%和1.0%。

Conclusion: 验证了推理原型是提升大模型泛化能力的基础。
...
### [16] [MinosEval: Distinguishing Factoid and Non-Factoid for Tailored Open-Ended QA Evaluation with LLMs](https://arxiv.org/abs/2506.15215)
...
<details>
  <summary>Details</summary>
Motivation: 开放性问题回答（QA）需要更长的答案和更细致的推理，传统评估方法（如ROUGE和BERTScore）难以捕捉语义相似性，而现有的基于LLM的评估方法缺乏直观解释性，且未区分问题类型。
...
### [17] [Lost in Variation? Evaluating NLI Performance in Basque and Spanish Geographical Variants](https://arxiv.org/abs/2506.15239)
...
Result: 性能下降主要源于语言变体本身，尤其是西部巴斯克语，与语言学理论一致。
...
### [18] [Research on Graph-Retrieval Augmented Generation Based on Historical Text Knowledge Graphs](https://arxiv.org/abs/2506.15241)
*Yang Fan,Zhang Qi,Xing Wenqian,Liu Chang,Liu Liu*

Main category: cs.CL

TL;DR: 论文提出Graph RAG框架，结合链式思维提示、自指令生成和过程监督，构建历史人物关系数据集，减少人工标注成本，并通过知识图谱与检索增强生成的协作机制提升模型性能。
...
Result: Xunzi-Qwen1.5-14B模型在关系抽取任务中表现最佳（F1=0.68），GraphRAG集成DeepSeek模型在开放域数据集上F1提升11%，有效减少幻觉现象。
...
### [19] [TopClustRAG at SIGIR 2025 LiveRAG Challenge](https://arxiv.org/abs/2506.15246)
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模网络语料库中的问答问题，提升答案的多样性和相关性。

Method: 采用混合检索策略和K-Means聚类，生成集群特定提示并通过LLM生成答案。
...
### [20] [Thunder-DeID: Accurate and Efficient De-identification Framework for Korean Court Judgments](https://arxiv.org/abs/2506.15266)
*Sungen Hahm,Heejin Kim,Gyuseong Lee,Hyunji Park,Jaejin Lee*

Main category: cs.CL

TL;DR: 论文提出Thunder-DeID框架，用于高效、合规地处理韩国法院判决的去标识化问题，并发布首个韩国法律数据集。


<details>
  <summary>Details</summary>
Motivation: 当前韩国法院判决的去标识化方法效率低且法律定义模糊，难以满足大规模处理需求。

Method: 构建首个韩国法律数据集，系统化分类个人身份信息，开发基于DNN的去标识化流程。

Result: 模型在法院判决去标识化任务中达到最优性能。

Conclusion: Thunder-DeID框架有效解决了去标识化的法律和技术挑战。
...
### [21] [Cohort Discovery: A Survey on LLM-Assisted Clinical Trial Recruitment](https://arxiv.org/abs/2506.15301)
...
<details>
  <summary>Details</summary>
Motivation: LLMs在通用NLP任务中表现优异，但在临床试验招募等关键领域应用有限，需要探索其潜力。

Method: 通过分析现有LLM辅助方法、评估基准及挑战，提出未来研究方向。

Result: LLMs在整合知识和推理方面具有潜力，但目前依赖专有模型和弱评估基准。

Conclusion: LLMs有望为临床试验匹配提供通用解决方案，但需改进评估框架和解决实际挑战。
...
### [22] [ConLID: Supervised Contrastive Learning for Low-Resource Language Identification](https://arxiv.org/abs/2506.15304)
*Negar Foroutan,Jakhongir Saydaliev,Ye Eun Kim,Antoine Bosselut*

Main category: cs.CL

TL;DR: 提出了一种监督对比学习方法，用于提升低资源语言的域外识别性能，效果显著。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言在语言识别（LID）中因数据不平衡和偏见导致的性能不佳问题。

Method: 采用监督对比学习（SCL）方法，学习域不变表示。

Result: 在域外数据上，低资源语言的LID性能提升了3.2%。

Conclusion: 该方法有效提升了低资源语言的识别能力。
...
### [24] [SANSKRITI: A Comprehensive Benchmark for Evaluating Language Models' Knowledge of Indian Culture](https://arxiv.org/abs/2506.15355)
...
Method: 通过构建SANSKRITI数据集，涵盖印度文化的16个关键属性，并在多种语言模型上测试其表现。
...
### [25] [COSMMIC: Comment-Sensitive Multimodal Multilingual Indian Corpus for Summarization and Headline Generation](https://arxiv.org/abs/2506.15372)
...
Method: 引入COSMMIC数据集，包含4,959篇文章-图像对和24,484条评论，使用LLama3和GPT-4等模型评估不同配置（仅文本、文本+评论、文本+图像、全组合）的效果。

Result: COSMMIC数据集有效整合了文本、图像和用户评论，为印度语言的NLP研究提供了资源，并确定了最佳配置。

Conclusion: COSMMIC数据集推动了印度语言的多模态和多语言摘要研究，促进了NLP领域的包容性。
...
### [26] [Targeted Lexical Injection: Unlocking Latent Cross-Lingual Alignment in Lugha-Llama via Early-Layer LoRA Fine-Tuning](https://arxiv.org/abs/2506.15415)
*Stanley Ngugi*
...
<details>
  <summary>Details</summary>
Motivation: 低资源语言在大型语言模型中的表现较差，主要由于数据稀缺和预训练中的代表性不足。本文旨在解决跨语言词汇对齐的挑战，以提升翻译和跨语言信息检索等任务的效果。

Method: 采用Targeted Lexical Injection（TLI）方法，结合Low-Rank Adaptation（LoRA）和对比学习目标，针对早期内部层（如Layer 2）的词汇对齐能力进行微调。

Result: TLI显著提升了623个斯瓦希里语-英语词对的词汇对齐效果，平均余弦相似度从0.3211提升至0.4113（+28.08%），并且在未见过的控制词对上表现出良好的泛化能力。

Conclusion: TLI通过高效利用模型的早期层知识，提供了一种参数高效且有效的方法，显著改善了低资源语言在大型语言模型中的词汇对齐能力。
...
### [27] [Understanding GUI Agent Localization Biases through Logit Sharpness](https://arxiv.org/abs/2506.15425)
*Xingjian Tao,Yiwei Wang,Yujun Cai,Zhicheng Yang,Jing Tang*

Main category: cs.CL

TL;DR: 提出了一种细粒度评估框架和Peak Sharpness Score（PSS）指标，用于量化多模态大语言模型（MLLMs）在GUI代理中的不确定性，并通过上下文感知裁剪技术提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管MLLMs在GUI代理中表现良好，但其系统性定位错误（幻觉）降低了可靠性，需要更精细的评估和改进方法。

Method: 1. 提出细粒度评估框架，将预测分为四类；2. 引入PSS指标量化模型不确定性；3. 提出训练无关的上下文感知裁剪技术。
...
### [28] [AgentGroupChat-V2: Divide-and-Conquer Is What LLM-Based Multi-Agent System Need](https://arxiv.org/abs/2506.15451)
*Zhouhong Gu,Xiaoxuan Zhu,Yin Cai,Hao Shen,Xingzhou Chen,Qingyi Wang,Jialin Li,Xiaoran Shi,Haoran Guo,Wenxuan Huang,Hongwei Feng,Yanghua Xiao,Zheyu Ye,Yao Hu,Shaosheng Cao*

Main category: cs.CL

TL;DR: AgentGroupChat-V2是一个基于大语言模型的多智能体系统框架，通过分治并行架构、自适应协作引擎和智能体组织优化策略，显著提升了复杂任务处理的性能和跨领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统在架构设计、跨领域泛化性和性能保障方面面临挑战，尤其是在任务复杂性和智能体数量增加时。

Method: 提出三个核心创新：分治并行架构、自适应协作引擎和智能体组织优化策略。

Result: 在多个领域表现出色，如GSM8K准确率91.50%，AIME准确率30.4%，HumanEval pass@1 79.20%，且在任务难度增加时优势更明显。

Conclusion: AgentGroupChat-V2为构建高效、通用的多智能体系统提供了全面解决方案，特别适用于复杂推理场景。
...
### [29] [RE-IMAGINE: Symbolic Benchmark Synthesis for Reasoning Evaluation](https://arxiv.org/abs/2506.15455)
*Xinnuo Xu,Rachel Lawrence,Kshitij Dubey,Atharva Pandey,Risa Ueno,Fabian Falck,Aditya V. Nori,Rahul Sharma,Amit Sharma,Javier Gonzalez*

Main category: cs.CL

TL;DR: RE-IMAGINE框架通过符号表示生成问题变体，评估LLMs是否依赖记忆而非真正推理能力。
...
Method: 基于因果阶梯理论，提出RE-IMAGINE框架，生成不同层次的问题变体，评估LLMs表现。
...
### [30] [Context-Informed Grounding Supervision](https://arxiv.org/abs/2506.15480)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在推理时简单附加上下文无法确保生成内容基于上下文的问题。

Method: 提出CINGS方法，训练时将相关上下文附加到响应中，但仅计算响应标记的损失。

Result: 在11个信息检索数据集上表现优于其他方法，视觉语言模型中减少了幻觉并保持事实一致性。

Conclusion: CINGS通过隐式鼓励模型依赖外部上下文，显著提升了生成内容的准确性，且不影响下游任务性能。
...
### [31] [SPARE: Single-Pass Annotation with Reference-Guided Evaluation for Automatic Process Supervision and Reward Modelling](https://arxiv.org/abs/2506.15498)
...
Conclusion: SPARE为过程监督提供了高效且高质量的解决方案，推动了LLM在多步推理任务中的应用。
...
### [32] [Enhancing Hyperbole and Metaphor Detection with Their Bidirectional Dynamic Interaction and Emotion Knowledge](https://arxiv.org/abs/2506.15504)
*Li Zheng,Sihang Wang,Hao Fei,Zuquan Peng,Fei Li,Jianming Fu,Chong Teng,Donghong Ji*

Main category: cs.CL

TL;DR: 提出了一种基于情感引导的双向动态交互框架（EmoBi），用于检测夸张和隐喻，通过情感分析和双向动态交互提升检测性能，实验表明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 夸张和隐喻在自然语言处理中具有重要意义，但由于其语义模糊和表达多样，检测具有挑战性。现有方法多关注表面特征，忽略了情感关联和修辞设备的隐含效果。

Method: EmoBi框架包括情感分析模块、基于情感的领域映射模块和双向动态交互模块，并通过验证机制确保准确性。

Result: 在四个数据集上EmoBi优于基线方法，F1分数在TroFi数据集上提升28.1%（夸张检测），在HYPO-L数据集上提升23.1%（隐喻检测）。
...
### [33] [Lessons from Training Grounded LLMs with Verifiable Rewards](https://arxiv.org/abs/2506.15522)
*Shang Hong Sim,Tej Deep Pala,Vernon Toh,Hai Leong Chieu,Amir Zadeh,Chuan Li,Navonil Majumder,Soujanya Poria*

Main category: cs.CL

TL;DR: 该论文探讨如何通过强化学习和内部推理提升大语言模型（LLMs）的生成可信度和引用准确性，提出GRPO方法，显著优于仅指令调优的模型。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在生成可信回答和正确引用时的常见问题，如遗漏明确答案、错误引用或拒绝回答。

Method: 使用GRPO（Group Relative Policy Optimization）方法，通过可验证的结果奖励（答案正确性、引用充分性和拒绝质量）训练模型，无需昂贵标注。

Result: 在多个数据集（ASQA、QAMPARI等）上，推理增强模型显著优于仅指令调优的模型，尤其在处理不可回答问题和生成引用充分的回答时。
...
### [34] [RATTENTION: Towards the Minimal Sliding Window Size in Local-Global Attention Models](https://arxiv.org/abs/2506.15545)
*Bailin Wang,Chang Lan,Chong Wang,Ruoming Pang*

Main category: cs.CL

TL;DR: RATTENTION是一种结合局部注意力和线性注意力的新方法，旨在解决局部注意力忽略窗口外令牌的问题，同时在短上下文和长上下文中实现性能与效率的平衡。
...
Result: 在3B和12B规模的预训练中，RATTENTION以512窗口大小匹配全注意力模型的性能，并在长上下文中表现更优。

Conclusion: RATTENTION在性能和效率上取得平衡，且不牺牲训练速度。
...
### [35] [Approximating Language Model Training Data from Weights](https://arxiv.org/abs/2506.15553)
*John X. Morris,Junjie Oscar Yin,Woojeong Kim,Vitaly Shmatikov,Alexander M. Rush*

Main category: cs.CL

TL;DR: 论文提出了一种从模型权重中近似训练数据的方法，通过梯度选择公共文本语料库中的高匹配数据，显著提升了模型性能。
...
Method: 提出梯度基方法，从大型公共文本语料库中选择高匹配数据，并验证其在分类和监督微调任务中的有效性。

Result: 在AG News分类任务中，性能从65%提升至80%，接近专家基准88%；在MSMARCO微调任务中，困惑度从3.3降至2.3。

Conclusion: 该方法能从公开数据中有效恢复有用信息，显著提升模型性能，接近专家水平。
...
### [36] [PredGen: Accelerated Inference of Large Language Models through Input-Time Speculation for Real-Time Speech Interaction](https://arxiv.org/abs/2506.15556)
...
Result: 模拟实验显示，PredGen可将延迟减少约2倍，额外计算成本极低。
...
### [37] [Gender Inclusivity Fairness Index (GIFI): A Multilevel Framework for Evaluating Gender Diversity in Large Language Models](https://arxiv.org/abs/2506.15568)
*Zhengyang Shan,Emily Ruth Diana,Jiawei Zhou*

Main category: cs.CL

TL;DR: 论文提出了一种新的性别包容性公平指数（GIFI），用于评估大型语言模型（LLMs）在处理二元和非二元性别时的公平性，并发现不同模型在性别包容性上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 以往研究主要关注二元性别，缺乏对非二元性别的评估，因此需要一种更全面的指标来衡量LLMs的性别包容性。

Method: 提出GIFI指标，通过多层次的评估（如代词测试、生成行为分析等）量化LLMs的性别包容性，并在22个开源和专有LLMs上进行测试。

Result: 研究发现不同LLMs在性别包容性上存在显著差异，揭示了模型对不同性别标识的偏见。

Conclusion: 研究强调了提升LLMs性别包容性的重要性，并为未来性别公平性研究提供了重要基准。
...
### [38] [SciVer: Evaluating Foundation Models for Multimodal Scientific Claim Verification](https://arxiv.org/abs/2506.15569)
*Chengye Wang,Yifei Shen,Zexi Kuang,Arman Cohan,Yilun Zhao*

Main category: cs.CL

TL;DR: SciVer是一个专门用于评估基础模型在多模态科学背景下验证声明能力的基准，包含3000个专家标注的示例，覆盖四种常见推理类型。实验发现当前模型与人类专家存在显著性能差距，并揭示了开源模型的关键局限性。
...
Method: SciVer基于1113篇科学论文构建了3000个专家标注的示例，评估了21种最先进的多模态基础模型，包括o4-mini、Gemini-2.5-Flash等，并分析了检索增强生成（RAG）和人工错误评估。

Result: 实验结果显示，当前模型与人类专家在SciVer上的表现存在显著差距，开源模型在多模态科学任务中存在关键局限性。

Conclusion: SciVer为多模态科学声明验证提供了首个基准，揭示了当前模型的不足，并为未来模型改进提供了关键见解。
...
### [39] [DiscoSG: Towards Discourse-Level Text Scene Graph Parsing through Iterative Graph Refinement](https://arxiv.org/abs/2506.15583)
...
<details>
  <summary>Details</summary>
Motivation: 现有的单句子场景图解析方法在多句子描述中表现不佳，无法处理跨句子指代等问题，导致下游任务性能下降。

Method: 提出DiscoSG任务，构建数据集DiscoSG-DS，并设计DiscoSG-Refiner方法，使用两个小型PLM分阶段生成和优化场景图。

Result: DiscoSG-Refiner在SPICE指标上比基线提升30%，推理速度比GPT-4快86倍，同时提升下游任务性能。
...
### [40] [WikiMixQA: A Multimodal Benchmark for Question Answering over Tables and Charts](https://arxiv.org/abs/2506.15594)
...
<details>
  <summary>Details</summary>
Motivation: 文档理解面临复杂布局和多模态信息的挑战，现有模型在长上下文视觉输入处理上的有效性尚不明确。

Method: 提出WikiMixQA基准，包含1000道多选题，基于4000篇维基百科页面的表格和图表，评估12种先进视觉语言模型。

Result: 专有模型在直接上下文下准确率约70%，但在长文档检索中显著下降；开源模型最高准确率仅27%，GPT-4-o表现最佳。

Conclusion: WikiMixQA为文档理解研究提供了重要基准，凸显长上下文多模态推理的挑战。
...
### [41] [From Model to Classroom: Evaluating Generated MCQs for Portuguese with Narrative and Difficulty Concerns](https://arxiv.org/abs/2506.15598)
*Bernardo Leite,Henrique Lopes Cardoso,Pedro Pinto,Abel Ferreira,Luís Abreu,Isabel Rangel,Sandra Monteiro*

Main category: cs.CL

TL;DR: 论文探讨了利用生成式AI自动生成葡萄牙语阅读理解多选题（MCQ）的可行性，评估了其质量和可靠性，并指出当前模型的优缺点。


<details>
  <summary>Details</summary>
Motivation: 手动创建不同难度和针对性的MCQ耗时且昂贵，生成式AI提供了自动化解决方案，但其质量和可靠性尚未充分研究，尤其在非英语语言中。

Method: 研究使用生成模型为葡萄牙语阅读理解生成MCQ，涵盖课程相关叙事元素和不同难度，并通过专家评审和学生反应的心理测量学分析评估质量。

Result: 当前模型生成的MCQ质量接近人工编写，但在语义清晰度和可回答性上存在问题，且干扰项设计仍需改进。

Conclusion: 生成式AI在MCQ生成中具有潜力，但需进一步优化语义清晰度和干扰项设计，以提升实用性。
...
### [42] [The Compositional Architecture of Regret in Large Language Models](https://arxiv.org/abs/2506.15617)
...
### [43] [Minding the Politeness Gap in Cross-cultural Communication](https://arxiv.org/abs/2506.15623)
*Yuka Machino,Matthias Hofer,Max Siegel,Joshua B. Tenenbaum,Robert D. Hawkins*

Main category: cs.CL

TL;DR: 研究探讨英美英语使用者对“quite”和“very”等强调词的不同理解，发现文化差异源于字面意义和话语成本的权衡。
...
Method: 通过三个实验和计算认知模型分析英美英语使用者对强调词的解释。

Result: 文化差异源于字面意义和话语成本的组合，而非单纯语义或礼貌规范。
...
### [44] [Revisiting Compositional Generalization Capability of Large Language Models Considering Instruction Following Ability](https://arxiv.org/abs/2506.15629)
*Yusuke Sakai,Hidetaka Kamigaito,Taro Watanabe*

Main category: cs.CL

TL;DR: 论文提出了Ordered CommonGen基准，用于评估大语言模型在指令遵循和组合泛化能力上的表现，发现模型在指定概念顺序时存在偏见和低多样性问题。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在生成符合指定概念顺序的句子时的能力，并评估其指令遵循和组合泛化能力。

Method: 提出Ordered CommonGen基准，通过有序覆盖率评估模型表现，并对36种大语言模型进行综合分析。

Result: 发现模型虽能理解指令意图，但存在对特定顺序模式的偏见，导致输出多样性低；最佳模型的指令遵循率仅约75%。

Conclusion: 大语言模型在指令遵循和组合泛化能力上仍需改进，Ordered CommonGen为评估提供了有效工具。
...
### [45] [Oldies but Goldies: The Potential of Character N-grams for Romanian Texts](https://arxiv.org/abs/2506.15650)
*Dana Lupsa,Sanda-Maria Avram*

Main category: cs.CL

TL;DR: 该研究通过评估六种机器学习技术，发现ANN模型在罗马尼亚文本作者归属任务中表现最佳，尤其是使用5-gram特征时。
...
Result: ANN模型表现最佳，尤其是使用5-gram特征时，部分情况下实现了完美分类。
...
### [46] [CC-LEARN: Cohort-based Consistency Learning](https://arxiv.org/abs/2506.15662)
...
Method: 提出基于同类问题的强化学习框架CC-Learn，结合准确性、检索奖励和拒绝惩罚的复合目标优化模型。
...
### [47] [Leaky Thoughts: Large Reasoning Models Are Not Private Thinkers](https://arxiv.org/abs/2506.15674)
...
Result: 增加推理步骤会提高模型谨慎性，但也导致更多隐私泄露。
...
TL;DR: 分析了21种机器翻译系统在性别模糊情况下的表现，发现大多数系统未能实现性别中立翻译，仅有少数系统采用了特定策略。


<details>
  <summary>Details</summary>
Motivation: 研究性别包容性机器翻译，以避免性别误判和代表性伤害，特别是在语法性别语言中保持性别模糊性。
...
### [49] [GenRecal: Generation after Recalibration from Large to Small Vision-Language Models](https://arxiv.org/abs/2506.15681)
*Byung-Kwan Lee,Ryo Hachiuma,Yong Man Ro,Yu-Chiang Frank Wang,Yueh-Hua Wu*

Main category: cs.CL

TL;DR: 论文提出了一种名为GenRecal的新框架，用于解决异构视觉语言模型（VLMs）之间的知识蒸馏问题，通过特征对齐和适应实现高效知识迁移。


<details>
  <summary>Details</summary>
Motivation: 由于异构VLMs在架构、词汇和分词方式上的差异，现有方法难以实现跨模型的高效知识蒸馏，限制了资源受限设备上的部署。

Method: GenRecal框架引入了一个Recalibrator模块，用于对齐和适应不同VLMs的特征表示，从而实现跨模型知识迁移。

Result: 实验表明，GenRecal在多个基准测试中显著提升性能，甚至超越大规模开源和闭源VLMs。

Conclusion: GenRecal为异构VLMs的知识蒸馏提供了一种通用解决方案，具有实际部署潜力。
...
### [50] [PhantomHunter: Detecting Unseen Privately-Tuned LLM-Generated Text via Family-Aware Learning](https://arxiv.org/abs/2506.15683)
...
### [51] [SemIRNet: A Semantic Irony Recognition Network for Multimodal Sarcasm Detection](https://arxiv.org/abs/2506.14791)
...
### [52] [Argus Inspection: Do Multimodal Large Language Models Possess the Eye of Panoptes?](https://arxiv.org/abs/2506.14805)
*Yang Yao,Lingyu Li,Jiaxin Song,Chiyu Chen,Zhenqi He,Yixu Wang,Xin Wang,Tianle Gu,Jie Li,Yan Teng,Yingchun Wang*

Main category: cs.CV

TL;DR: 论文提出Argus Inspection多模态基准和Eye of Panoptes框架，用于评估MLLMs的细粒度视觉感知和常识因果推理能力，实验显示当前模型性能仍有提升空间。
...
Method: 引入Argus Inspection多模态基准（含两个难度级别）和Eye of Panoptes框架（结合Sigmoid度量与指示函数），对26种主流MLLMs进行评估。

Result: 实验结果显示，视觉细粒度推理的最高性能仅为0.46，表明模型仍有较大改进空间。
...
### [53] [A Hybrid ConvNeXt-EfficientNet AI Solution for Precise Falcon Disease Detection](https://arxiv.org/abs/2506.14816)
...
Method: 采用ConvNeXt和EfficientNet混合模型，针对三种疾病（正常、肝病、曲霉病）进行分类。

Result: 混合模型在准确率、精确率、召回率和F1分数上优于传统方法和单一模型。

Conclusion: 该混合AI模型为猎鹰疾病检测提供了高效工具，推动了AI在鸟类医疗中的应用。
...
### [54] [ViLLa: A Neuro-Symbolic approach for Animal Monitoring](https://arxiv.org/abs/2506.14823)
*Harsha Koduri*
...
TL;DR: 论文提出GraphGSOcc模型，结合语义和几何图Transformer解决3D高斯溅射方法中的特征聚合和边界模糊问题。
...
Result: 在SurroundOcc数据集上mIoU达24.10%，GPU内存降至6.1GB，性能提升明显。
...
### [56] [DAVID-XR1: Detecting AI-Generated Videos with Explainable Reasoning](https://arxiv.org/abs/2506.14827)
...
TL;DR: 论文提出了DAVID-X数据集和DAVID-XR1模型，用于透明、可验证地检测AI生成视频，并提供细粒度的缺陷分析和解释。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成视频的普及，现有方法仅提供二分类检测，缺乏透明性和解释性，无法满足审计和用户需求。

Method: 构建DAVID-X数据集，包含缺陷级时空标注和书面解释；开发DAVID-XR1模型，结合视觉推理链（缺陷分类、时空定位和自然语言解释）。
...
### [57] [Recent Advances in Multi-Agent Human Trajectory Prediction: A Comprehensive Review](https://arxiv.org/abs/2506.14831)
...
### [58] [ArchShapeNet:An Interpretable 3D-CNN Framework for Evaluating Architectural Shapes](https://arxiv.org/abs/2506.14832)
*Jun Yin,Jing Zhong,Pengyu Zeng,Peilin Li,Zixuan Dai,Miao Zhang,Shuai Lu*

Main category: cs.CV

TL;DR: 论文分析了人类设计与机器生成的3D建筑形式差异，提出ArchShapeNet模型，性能优于人类专家，并揭示了各自优势。
...
Conclusion: 揭示了人类设计在空间组织等方面的优势，为生成工具改进提供方向。
...
### [59] [Real-Time, Low-Latency Surveillance Using Entropy-Based Adaptive Buffering and MobileNetV2 on Edge Devices](https://arxiv.org/abs/2506.14833)
*Poojashree Chandrashekar Pankaj M Sajjanar*

Main category: cs.CV

TL;DR: 提出了一种适用于资源受限环境的高性能、低延迟视频监控系统，结合熵自适应帧缓冲算法与MobileNetV2，实现高吞吐量与低延迟。
...
Conclusion: 该系统具有可扩展性、低成本及符合严格数据隐私法规的特点，适用于智能城市或嵌入式安全架构。
...
### [60] [MonoVQD: Monocular 3D Object Detection with Variational Query Denoising and Self-Distillation](https://arxiv.org/abs/2506.14835)
...
<details>
  <summary>Details</summary>
Motivation: 单目3D检测中的DETR架构存在性能限制，MonoVQD旨在通过创新机制提升其表现。
...
### [61] [Improved Iterative Refinement for Chart-to-Code Generation via Structured Instruction](https://arxiv.org/abs/2506.14837)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在图表到代码生成任务中表现不佳，需要更精确的视觉理解和代码翻译能力。
...
### [62] [PictSure: Pretraining Embeddings Matters for In-Context Learning Image Classifiers](https://arxiv.org/abs/2506.14842)
...
<details>
  <summary>Details</summary>
Motivation: 在数据稀缺领域，构建图像分类模型困难，ICL成为少样本图像分类的有前景方法，但现有研究忽视了图像嵌入模型的关键作用。

Method: 提出PictSure框架，系统研究视觉编码器类型、预训练目标和微调策略对少样本分类性能的影响。

Result: 实验表明，嵌入模型的预训练方式对训练成功和跨域性能至关重要，PictSure在跨域任务中优于现有ICL模型。
...
### [63] [Finding Optimal Kernel Size and Dimension in Convolutional Neural Networks An Architecture Optimization Approach](https://arxiv.org/abs/2506.14846)
*Shreyas Rajeev,B Sathish Babu*

Main category: cs.CV

TL;DR: 本文提出了一种基于数学和实证的框架BKSEF，用于优化CNN中逐层的核大小选择，平衡信息增益、计算效率和准确性。实验表明，该方法在多个数据集上显著提升了模型性能并降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 核大小选择在CNN设计中至关重要，但常被忽视。本文旨在通过BKSEF框架优化核大小选择，以提升模型性能和效率。

Method: 提出BKSEF框架，结合信息论、信号处理和学习理论，逐层确定最优核大小。

Result: 在多个数据集上，BKSEF引导的模型实现了最高3.1%的准确率提升和42.8%的FLOPs减少。实际案例验证了其在医疗图像分类和交通标志识别中的有效性。

Conclusion: BKSEF证明核大小可作为可优化参数，为CNN设计提供了理论和实践支持，适用于神经架构搜索和实时系统。
...
Method: 利用深度神经网络学习特征，结合目标检测技术，自动识别关键帧并标注。

Result: 方法准确性接近人工标注，成本节省2倍，仅需人工验证5%的帧。
...
### [65] [Peering into the Unknown: Active View Selection with Neural Uncertainty Maps for 3D Reconstruction](https://arxiv.org/abs/2506.14856)
...
<details>
  <summary>Details</summary>
Motivation: 解决3D重建中如何选择最有信息量的视角以减少冗余并提高效率的问题。

Method: 使用轻量级神经网络UPNet预测不确定性图，通过启发式方法选择最有信息量的视角。

Result: 使用一半的视角即可达到与上限相当的精度，计算开销大幅降低（400倍加速，资源消耗减少50%以上）。
...
### [66] [DETONATE: A Benchmark for Text-to-Image Alignment and Kernelized Direct Preference Optimization](https://arxiv.org/abs/2506.14903)
...
TL;DR: 该论文提出了一种名为DPO-Kernels的新方法，用于增强文本到图像（T2I）模型的对齐能力，包括混合损失、核化表示和发散选择三个维度，并引入了DETONATE基准和AQI指标。


<details>
  <summary>Details</summary>
Motivation: 确保T2I模型生成的图像准确反映用户意图，同时保持安全性和公平性。

Method: 提出DPO-Kernels方法，包括混合损失、核化表示和发散选择三个创新点。

Result: 通过DETONATE基准和AQI指标验证了方法的有效性，展示了强泛化能力。

Conclusion: DPO-Kernels在T2I模型中对齐任务中表现出色，为安全性和公平性提供了新工具。
...
### [67] [PeRL: Permutation-Enhanced Reinforcement Learning for Interleaved Vision-Language Reasoning](https://arxiv.org/abs/2506.14907)
...
Method: 提出PeRL方法，包括图像序列排列和多阶段策略，以及用于优化探索-利用权衡的轨迹过滤机制。

Result: 在5个多图像基准和3个单图像基准上，PeRL显著优于现有方法，达到多图像任务的领先性能。
...
### [68] [Frequency-Calibrated Membership Inference Attacks on Medical Image Diffusion Models](https://arxiv.org/abs/2506.14919)
*Xinkai Zhao,Yuta Tokuoka,Junichiro Iwasawa,Keita Oda*

Main category: cs.CV

TL;DR: 提出了一种针对医学图像扩散模型的频率校准重建误差（FCRE）方法，用于改进成员推理攻击（MIA），通过聚焦中频范围重建误差，减少图像固有难度的影响。
...
### [69] [Vision Transformers for End-to-End Quark-Gluon Jet Classification from Calorimeter Images](https://arxiv.org/abs/2506.14934)
*Md Abrar Jahin,Shahriar Soudeep,Arian Rahman Aditta,M. F. Mridha,Nafiz Fahad,Md. Jakir Hossen*

Main category: cs.CV

TL;DR: 本文探讨了Vision Transformer（ViT）及其混合模型在区分夸克和胶子喷注中的应用，发现其性能优于传统CNN方法。
...
### [70] [Advances in Compliance Detection: Novel Models Using Vision-Based Tactile Sensors](https://arxiv.org/abs/2506.14980)
*Ziteng Li,Malte Kuhlmann,Ilana Nisky,Nicolás Navarro-Guerrero*

Main category: cs.CV

TL;DR: 论文提出基于LRCN和Transformer的模型，利用RGB触觉图像和GelSight传感器信息，显著提高了合规性预测的准确性。
...
Result: 模型在多种指标下表现优异，显著优于基线；发现传感器与物体合规性估计的关联性。

Conclusion: 模型有效提升合规性预测精度；较硬物体的合规性估计更具挑战性。
...
### [71] [Hyper-Local Deformable Transformers for Text Spotting on Historical Maps](https://arxiv.org/abs/2506.15010)
...
<details>
  <summary>Details</summary>
Motivation: 历史地图中的文本提供了丰富的地理、历史和文化信息，但现有方法难以有效提取，尤其是针对复杂背景和多样化的地图风格。

Method: PALETTE引入超局部采样模块学习文本实例的边界点和字符特征，并结合超局部位置嵌入增强空间交互；同时提出SynthMap+自动生成合成地图数据用于训练。

Result: PALETTE结合SynthMap+在历史地图数据集上表现优于现有方法，已成功处理6万张地图并生成1亿多个文本标签。
...
### [72] [Break Stylistic Sophon: Are We Really Meant to Confine the Imagination in Style Transfer?](https://arxiv.org/abs/2506.15033)
...
TL;DR: StyleWallfacer是一个统一的训练和推理框架，通过语义风格注入、数据增强和扩散过程实现高质量的风格迁移和文本驱动风格化。


<details>
  <summary>Details</summary>
Motivation: 解决传统风格迁移方法的问题，统一不同任务的框架，实现艺术家级别的风格迁移和文本驱动风格化。

Method: 1. 基于BLIP的语义风格注入；2. 基于人类反馈的数据增强；3. 无训练的三重扩散过程。

Result: 实现了高质量的风格迁移和文本驱动风格化，首次在风格迁移中完成图像颜色编辑。

Conclusion: StyleWallfacer框架在风格迁移领域具有革命性意义，能够高效且无偏移地注入风格知识。
...
### [73] [Enhancing Vector Quantization with Distributional Matching: A Theoretical and Empirical Study](https://arxiv.org/abs/2506.15078)
...
Method: 使用Wasserstein距离对齐特征和码向量分布。
...
### [74] [SynPo: Boosting Training-Free Few-Shot Medical Segmentation via High-Quality Negative Prompts](https://arxiv.org/abs/2506.15153)
...
### [75] [Enhancing point cloud analysis via neighbor aggregation correction based on cross-stage structure correlation](https://arxiv.org/abs/2506.15160)
...
Conclusion: PDSA模块有效解决了现有问题，实验和可视化结果证明了其有效性和合理性。
...
### [76] [Echo-DND: A dual noise diffusion model for robust and precise left ventricle segmentation in echocardiography](https://arxiv.org/abs/2506.15166)
...
### [77] [ReSeDis: A Dataset for Referring-based Object Search across Large-Scale Image Collections](https://arxiv.org/abs/2506.15180)
*Ziling Huang,Yidan Zhang,Shin'ichi Satoh*

Main category: cs.CV

TL;DR: ReSeDis统一了大规模检索与像素级定位，提出新任务和基准，支持端到端多模态搜索系统开发。


<details>
  <summary>Details</summary>
Motivation: 现有技术无法同时解决图像检索和精确定位的双重问题，ReSeDis填补了这一空白。

Method: 引入ReSeDis任务，结合检索与定位，设计新基准和评估指标，提供零基线模型。

Result: ReSeDis为多模态搜索系统提供了现实测试平台，显示未来研究空间。
...
### [78] [Conquering the Retina: Bringing Visual in-Context Learning to OCT](https://arxiv.org/abs/2506.15200)
...
<details>
  <summary>Details</summary>
Motivation: 尽管专业模型在特定临床任务中表现优异，但其适用性受限且开发成本高。通用模型则允许动态定义任务，无需特定开发。
...
### [79] [Privacy-Shielded Image Compression: Defending Against Exploitation from Vision-Language Pretrained Models](https://arxiv.org/abs/2506.15201)
*Xuelin Shen,Jiayin Xu,Kangsheng Yin,Wenhan Yang*

Main category: cs.CV

TL;DR: 本文提出了一种名为PSIC的灵活编码方法，通过在图像压缩阶段实施防御，保护用户隐私，同时保留原始压缩功能。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言预训练模型语义理解能力的提升，公开图片易被搜索引擎等工具利用，亟需保护用户隐私。

Method: 提出PSIC方法，生成具有多种解码选项的比特流；设计CLTG模块和UAEO优化函数，结合自适应多目标优化策略。

Result: 实验证明该方法在多个下游任务中有效，既能保护隐私，又能保持图像感知质量。
...
### [80] [DM-FNet: Unified multimodal medical image fusion via diffusion process-trained encoder-decoder](https://arxiv.org/abs/2506.15218)
*Dan He,Weisheng Li,Guofen Wang,Yuping Huang,Shiqiang Liu*

Main category: cs.CV

TL;DR: 提出了一种基于两阶段扩散模型的融合网络（DM-FNet），用于多模态医学图像融合，通过增强特征识别和自适应处理不同模态图像，显著提升了融合图像的质量和信息密度。
...
Conclusion: DM-FNet通过优化网络结构和混合损失函数，显著提升了多模态医学图像融合的质量和信息密度。
...
### [81] [video-SALMONN 2: Captioning-Enhanced Audio-Visual Large Language Models](https://arxiv.org/abs/2506.15220)
...
Result: 模型仅需70亿参数，在视频字幕任务中超越GPT-4o和Gemini-1.5-Pro，错误率降低28%。

Conclusion: 视频-SALMONN 2在视频字幕生成任务中表现优异，方法高效且性能领先。
...
### [82] [Convolutional Feature Enhancement and Attention Fusion BiFPN for Ship Detection in SAR Images](https://arxiv.org/abs/2506.15231)
*Liangjie Meng,Danxia Li,Jinrong He,Lili Ma,Zhixin Li*

Main category: cs.CV

TL;DR: 论文提出了一种名为C-AFBiFPN的特征增强与融合框架，用于解决SAR船舶检测中的尺度变化、小目标噪声和复杂背景问题。


<details>
  <summary>Details</summary>
Motivation: SAR船舶检测面临船舶尺度差异大、小目标与噪声混杂以及近岸复杂背景等挑战，需要提升检测精度和鲁棒性。

Method: C-AFBiFPN结合了卷积特征增强模块（CFE）和创新的AFBiFPN网络，后者整合了BiFormer注意力机制，以增强局部细节捕捉和全局特征融合能力。
...
### [83] [RA-NeRF: Robust Neural Radiance Field Reconstruction with Accurate Camera Pose Estimation under Complex Trajectories](https://arxiv.org/abs/2506.15242)
*Qingsong Yan,Qiang Wang,Kaiyong Zhao,Jie Chen,Bo Li,Xiaowen Chu,Fei Deng*

Main category: cs.CV

TL;DR: RA-NeRF提出了一种新方法，通过结合光流驱动的姿态调节和隐式姿态滤波，显著提高了复杂相机轨迹下的姿态估计精度和场景重建质量。
...
Method: 结合NeRF的光度一致性和光流驱动的姿态调节，并引入隐式姿态滤波器。

Result: 在Tanks&Temple和NeRFBuster数据集上，RA-NeRF在姿态估计和视觉质量上均达到最优。

Conclusion: RA-NeRF在复杂相机轨迹下表现出高效性和鲁棒性。
...
### [84] [Retrospective Memory for Camouflaged Object Detection](https://arxiv.org/abs/2506.15244)
*Chenxi Zhang,Jiayun Wu,Qing Zhang,Yazhe Zhai,Youwei Pang*

Main category: cs.CV

TL;DR: 提出了一种名为RetroMem的COD架构，通过整合历史知识动态调整伪装模式感知和推理，显著提升模型性能。
...
### [85] [Domain Adaptation for Image Classification of Defects in Semiconductor Manufacturing](https://arxiv.org/abs/2506.15260)
*Adrian Poniatowski,Natalie Gentner,Manuel Barusco,Davide Dalle Pezze,Samuele Salti,Gian Antonio Susto*

Main category: cs.CV

TL;DR: 论文探讨了在半导体领域应用领域自适应（DA）技术，特别是在半监督和无监督设置下，以减少人工标注和模型重新训练的需求。作者提出了一种改进的CycleGAN模型（DBACS），并在真实电子显微镜图像上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 半导体行业对快速上市和高质量的需求日益增长，而领域自适应技术能够减少资源消耗并提高效率，因此研究其在半导体领域的应用具有重要意义。

Method: 作者提出了一种基于CycleGAN的DBACS模型，通过引入额外的损失项来提升性能。该方法在半监督和无监督设置下进行了测试。

Result: 实验结果表明，DBACS方法在真实电子显微镜图像上表现优异，证明了其在半导体领域的实用性和有效性。

Conclusion: DBACS方法在半导体领域的领域自适应任务中表现出色，为减少人工标注和计算成本提供了有效解决方案。
...
### [86] [MSNeRV: Neural Video Representation with Multi-Scale Feature Fusion](https://arxiv.org/abs/2506.15276)
*Jun Zhu,Xinfeng Zhang,Lv Tang,JunHao Jiang*

Main category: cs.CV

TL;DR: MSNeRV是一种多尺度特征融合框架，用于解决基于隐式神经表示（INR）的视频压缩方法在细节密集和快速变化内容上的不足。通过时间窗口、GoP级网格和多尺度空间解码器等方法，显著提升了视频表示和压缩性能。


<details>
  <summary>Details</summary>
Motivation: 现有INR方法在细节密集和快速变化的视频内容上表现不佳，主要原因是网络特征利用不足和缺乏视频特定设计。
...
### [87] [BCRNet: Enhancing Landmark Detection in Laparoscopic Liver Surgery via Bezier Curve Refinement](https://arxiv.org/abs/2506.15279)
*Qian Li,Feng Liu,Shuojue Yang,Daiyun Shen,Yueming Jin*

Main category: cs.CV

TL;DR: 论文提出BCRNet，通过Bezier曲线细化策略显著提升腹腔镜肝脏手术中关键解剖标志的检测精度。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜肝脏手术中准确识别解剖结构具有挑战性，AR系统通过2D-3D配准整合MRI/CT与腹腔镜图像，但需精确检测曲线标志。

Method: BCRNet包含多模态特征提取模块（MFE）、自适应曲线提案初始化（ACPI）和分层曲线细化（HCR），通过多阶段迭代优化Bezier曲线。
...
### [88] [AI-driven visual monitoring of industrial assembly tasks](https://arxiv.org/abs/2506.15285)
...
<details>
  <summary>Details</summary>
Motivation: 现有商业解决方案需要固定设置或标记，限制了灵活性。ViMAT旨在解决这一问题。

Method: ViMAT结合感知模块（从多视角视频提取视觉数据）和推理模块（基于观察和任务知识推断动作）。

Result: 在LEGO组件更换和液压模具重组任务中验证了ViMAT的有效性。

Conclusion: ViMAT在部分和不确定的视觉观察场景中表现出色。
...
### [89] [MEGC2025: Micro-Expression Grand Challenge on Spot Then Recognize and Visual Question Answering](https://arxiv.org/abs/2506.15298)
*Xinqi Fan,Jingting Li,John See,Moi Hoon Yap,Wen-Huang Cheng,Xiaobai Li,Xiaopeng Hong,Su-Jing Wang,Adrian K. Davision*

Main category: cs.CV

TL;DR: 论文探讨了面部微表情（MEs）的识别、检测和生成，提出结合检测与识别的统一任务（ME-STR）和基于多模态大语言模型的视觉问答任务（ME-VQA）。
...
Result: MEGC 2025挑战赛将评估这两种任务的表现，并发布排行榜。
...
### [90] [MapFM: Foundation Model-Driven HD Mapping with Multi-Task Contextual Learning](https://arxiv.org/abs/2506.15313)
...
Result: 模型显著提升了特征表示质量和预测准确性，生成了更高质量的矢量高清地图。
...
### [91] [OpenPath: Open-Set Active Learning for Pathology Image Classification via Pre-trained Vision-Language Models](https://arxiv.org/abs/2506.15318)
...
<details>
  <summary>Details</summary>
Motivation: 病理图像分类需要大规模标注数据，但标注成本高。传统主动学习方法在开集场景下效率低，OpenPath旨在解决这一问题。

Method: OpenPath结合任务特定提示和多样性信息采样策略（PIS和EGSS），从无标注池中选择高信息量和纯净样本。

Result: 在两个公共病理图像数据集上，OpenPath显著优于现有开集主动学习方法，提高了模型性能。

Conclusion: OpenPath通过高效样本选择和避免OOD数据，为病理图像分类提供了一种有效的主动学习解决方案。
...
### [92] [Open-World Object Counting in Videos](https://arxiv.org/abs/2506.15368)
...
<details>
  <summary>Details</summary>
Motivation: 解决拥挤场景中目标对象因遮挡或相似性导致的重复计数和重现识别问题。
...
### [93] [Unsupervised Pelage Pattern Unwrapping for Animal Re-identification](https://arxiv.org/abs/2506.15369)
...
Conclusion: 该方法通过几何感知的纹理映射显著提升了复杂姿态下的个体重识别性能。
...
### [94] [When Model Knowledge meets Diffusion Model: Diffusion-assisted Data-free Image Synthesis with Alignment of Domain and Class](https://arxiv.org/abs/2506.15381)
...
Method: 提出DDIS方法，结合扩散模型作为图像先验，引入领域对齐指导（DAG）和优化类对齐标记（CAT）。
...
### [95] [NERO: Explainable Out-of-Distribution Detection with Neuron-level Relevance](https://arxiv.org/abs/2506.15404)
...
<details>
  <summary>Details</summary>
Motivation: 在医学影像领域，确保深度学习模型的可靠性至关重要，尤其是识别分布外（OOD）样本以标记潜在异常。现有方法可能无法完全捕捉OOD多样性，因此需要更有效的解决方案。

Method: 提出NERO机制，通过聚类神经元级相关性形成代表性中心点，并引入相关性距离度量新样本与中心点的偏差。结合缩放相关性和特征范数优化性能。

Result: 在胃肠道影像基准测试（Kvasir和GastroVision）中，NERO优于现有OOD检测方法。

Conclusion: NERO通过神经元级相关性和距离度量显著提升了OOD检测能力，同时具备可解释性。
...
### [96] [Hunyuan3D 2.1: From Images to High-Fidelity 3D Assets with Production-Ready PBR Material](https://arxiv.org/abs/2506.15442)
...
TL;DR: 本文介绍了Hunyuan3D 2.1系统，提供了从数据处理到3D模型生成的完整教程，旨在降低3D AIGC的门槛。


<details>
  <summary>Details</summary>
Motivation: 3D AIGC领域复杂且难以普及，本文希望通过Hunyuan3D 2.1的案例研究，简化3D模型的生成流程。

Method: 系统包含Hunyuan3D-DiT（形状生成）和Hunyuan3D-Paint（纹理合成），教程涵盖数据处理、模型架构、训练策略、评估指标和部署。

Result: 通过教程，用户可以掌握生成高分辨率、带纹理的3D资产的技能。

Conclusion: 教程为开发适用于游戏、虚拟现实和工业设计的3D生成模型提供了实用指南。
...
### [97] [Multimodal Large Language Models for Medical Report Generation via Customized Prompt Tuning](https://arxiv.org/abs/2506.15477)
...
Method: MRG-LLM结合冻结的大语言模型和可学习的视觉编码器，通过条件仿射变换生成实例特定的提示，提出两种实现方式：提示级和提示簿级定制。

Result: 在IU X-ray和MIMIC-CXR数据集上的实验表明，MRG-LLM在医学报告生成任务中达到最先进性能。

Conclusion: MRG-LLM通过动态提示定制机制，有效提升了医学影像报告生成的精度和针对性。
...
### [98] [GenHOI: Generalizing Text-driven 4D Human-Object Interaction Synthesis for Unseen Objects](https://arxiv.org/abs/2506.15483)
*Shujia Li,Haiyu Zhang,Xinyuan Chen,Yaohui Wang,Yutong Ban*

Main category: cs.CV

TL;DR: GenHOI是一个两阶段框架，用于生成未见物体的高保真4D人-物交互序列，减少对大规模4D数据集的依赖。


<details>
  <summary>Details</summary>
Motivation: 由于大规模4D人-物交互数据集的稀缺，现有方法难以推广到未见物体并生成高质量4D序列。
...
Conclusion: GenHOI通过两阶段设计和接触感知技术，有效解决了4D人-物交互生成的挑战。
...
### [99] [NTIRE 2025 Image Shadow Removal Challenge Report](https://arxiv.org/abs/2506.15524)
...
TL;DR: NTIRE 2025阴影去除挑战赛的总结，306名参与者中17支团队提交了解决方案，分为重建保真度和视觉感知两个评估轨道。
...
### [100] [CLAIM: Clinically-Guided LGE Augmentation for Realistic and Diverse Myocardial Scar Synthesis and Segmentation](https://arxiv.org/abs/2506.15549)
...
Result: CLAIM生成的瘢痕模式解剖学一致，与真实瘢痕分布的Dice相似性高于基线模型。
...
### [101] [RaCalNet: Radar Calibration Network for Sparse-Supervised Metric Depth Estimation](https://arxiv.org/abs/2506.15560)
*Xingrui Qin,Wentao Zhao,Chuan Cao,Yihe Niu,Houcheng Jiang,Jingchuan Wang*

Main category: cs.CV

TL;DR: RaCalNet是一种无需密集监督的毫米波雷达深度估计框架，通过稀疏LiDAR监督实现高精度深度预测，性能优于现有密集监督方法。
...
Method: 通过稀疏雷达点校准和优化构建深度先验，指导单目深度预测，无需密集监督。

Result: 在ZJU-4DRadarCam数据集和实际场景中，RMSE分别降低35.30%和34.89%。

Conclusion: RaCalNet在稀疏监督下实现了优于密集监督方法的性能，提升了结构一致性和细节保留。
...
### [102] [Control and Realism: Best of Both Worlds in Layout-to-Image without Training](https://arxiv.org/abs/2506.15563)
*Bonan Li,Yinhan Hu,Songhua Liu,Xinchao Wang*

Main category: cs.CV

TL;DR: WinWinLay提出了一种无需训练的布局到图像生成方法，通过非局部注意力能量函数和自适应更新策略，提升了控制精度和真实感。
...
Method: 采用非局部注意力能量函数优化注意力分布，结合基于Langevin动力学的自适应更新规则，避免域外伪影。

Result: 实验表明WinWinLay在元素布局控制和视觉真实感上优于现有方法。

Conclusion: WinWinLay通过理论改进和实用策略，显著提升了布局到图像生成的质量。
...
### [103] [Show-o2: Improved Native Unified Multimodal Models](https://arxiv.org/abs/2506.15564)
...
<details>
  <summary>Details</summary>
Motivation: 构建一个统一的多模态模型，能够同时处理文本、图像和视频任务，提升多模态理解和生成的能力。

Method: 基于3D因果变分自编码器空间，通过双路径空间（时空）融合构建统一视觉表示，并采用两阶段训练方法。

Result: Show-o2模型在多种多模态任务中表现出色，支持文本、图像和视频的生成与理解。

Conclusion: Show-o2展示了在多模态任务中的强大能力，为未来的多模态研究提供了新的方向。
...
### [104] [Baltimore Atlas: FreqWeaver Adapter for Semi-supervised Ultra-high Spatial Resolution Land Cover Classification](https://arxiv.org/abs/2506.15565)
*Junhao Wu,Aboagye-Ntow Stephen,Chuyuan Wang,Gang Chen,Xin Huang*

Main category: cs.CV

TL;DR: 提出了一种参数高效的半监督分割框架，用于0.3米空间分辨率图像，结合SAM2知识和遥感专用FreqWeaver适配器，提升细粒度细节建模，仅占模型总参数的5.96%。


<details>
  <summary>Details</summary>
Motivation: 超高空间分辨率土地覆盖分类因像素级标注成本高、尺度变化大及大规模视觉模型适应性有限而具挑战性。现有方法多依赖标注数据且适用于1米分辨率，实际应用需弱监督下处理更高分辨率图像。

Method: 提出半监督分割框架，利用SAM2知识并引入FreqWeaver适配器，优化细粒度细节建模，同时保持轻量设计。

Result: 方法在未标注数据上表现稳健，结构一致性优越，比现有参数高效策略提升1.78%，比最先进高分辨率遥感分割方法提升3.44%。

Conclusion: 该框架在超高分辨率遥感图像分割中表现出色，参数高效且适应性强。
...
### [105] [A Unified Graph-based Framework for Scalable 3D Tree Reconstruction and Non-Destructive Biomass Estimation from Point Clouds](https://arxiv.org/abs/2506.15577)
*Di Wang,Shi Li*

Main category: cs.CV

TL;DR: 提出了一种基于图的新框架，用于大规模点云的端到端处理，实现森林地上生物量（AGB）的无损估计，显著减少了对专业预处理工具的依赖。


<details>
  <summary>Details</summary>
Motivation: 当前QSM方法在个体树估计、依赖高质量点云数据和复杂预处理步骤方面存在局限性，限制了其可扩展性和实际应用。

Method: 采用创新的基于图的流程，集成树分割、叶木分离和3D骨架重建，通过路径和抽象等图操作进行树拓扑推理。

Result: 在多种条件下（如叶覆盖、数据源）验证，表现优异，叶覆盖场景相对误差约20%，低密度ULS数据集约30%。
...
### [106] [One-Step Diffusion for Detail-Rich and Temporally Consistent Video Super-Resolution](https://arxiv.org/abs/2506.15591)
*Yujing Sun,Lingchen Sun,Shuaizheng Liu,Rongyuan Wu,Zhengqiang Zhang,Lei Zhang*

Main category: cs.CV

TL;DR: 论文提出了一种双LoRA学习（DLoRAL）范式，通过结合跨帧检索（CFR）模块和一致性-LoRA（C-LoRA）模块，在保持时间一致性的同时增强视频超分辨率的空间细节。


<details>
  <summary>Details</summary>
Motivation: 现有基于稳定扩散（SD）的真实视频超分辨率（Real-VSR）方法常因时间一致性牺牲空间细节，导致视觉质量不佳。论文旨在解决这一问题。

Method: 提出DLoRAL范式，分两阶段训练：1）通过CFR和C-LoRA提取时间一致性先验；2）固定CFR和C-LoRA，训练D-LoRA增强空间细节并保持时间一致性。两阶段交替优化。

Result: 实验表明，DLoRAL在精度和速度上均表现优异，能高效生成细节丰富且时间一致的视频。

Conclusion: DLoRAL通过双LoRA学习范式，成功实现了在单步扩散中同时保持时间一致性和增强空间细节的目标。
...
### [107] [Mono-Modalizing Extremely Heterogeneous Multi-Modal Medical Image Registration](https://arxiv.org/abs/2506.15596)
*Kyobin Choo,Hyunkyung Han,Jinyeong Kim,Chanyong Yoon,Seong Jae Hwang*

Main category: cs.CV

TL;DR: 提出了一种名为M2M-Reg的新框架，用于解决多模态图像配准中因模态差异大而导致传统方法失效的问题，通过单模态相似性训练模型，并引入GradCyCon正则化器提升效果。


<details>
  <summary>Details</summary>
Motivation: 临床实践中，功能成像（如PET、FA）常需与结构成像（如MRI、CT）配准，但传统无监督方法因模态差异大而难以学习可靠的空间映射。
...
### [108] [BoxFusion: Reconstruction-Free Open-Vocabulary 3D Object Detection via Real-Time Multi-View Box Fusion](https://arxiv.org/abs/2506.15610)
...
### [109] [HOIDiNi: Human-Object Interaction through Diffusion Noise Optimization](https://arxiv.org/abs/2506.15625)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在真实感和物理正确性之间的权衡问题，提出一种既能保证接触精度又能保持运动自然性的方法。

Method: 采用两阶段优化：对象中心阶段选择手-物接触位置，人体中心阶段优化全身运动，结合扩散噪声优化（DNO）技术。

Result: 在GRAB数据集上，HOIDiNi在接触精度、物理有效性和整体质量上均优于现有方法。
...
### [110] [FindingDory: A Benchmark to Evaluate Memory in Embodied Agents](https://arxiv.org/abs/2506.15635)
*Karmesh Yadav,Yusuf Ali,Gunshi Gupta,Yarin Gal,Zsolt Kira*

Main category: cs.CV

TL;DR: 论文提出了一种新的基准测试，用于评估大视觉语言模型在长期记忆任务中的表现，特别是在机器人控制中的应用。


<details>
  <summary>Details</summary>
Motivation: 现有的大视觉语言模型在处理长期记忆任务时效率不足，且缺乏针对具体机器人控制任务的评估标准。

Method: 引入了一个基于Habitat模拟器的基准测试，包含60个需要长期记忆和上下文感知的任务，并提出了结合视觉语言模型与导航策略的基线方法。

Result: 基准测试能够有效评估模型在长期记忆任务中的表现，并指出了改进方向。

Conclusion: 该基准测试为未来研究提供了可扩展的评估工具，有助于提升视觉语言模型在机器人控制中的应用能力。
...
### [111] [Demystifying the Visual Quality Paradox in Multimodal Large Language Models](https://arxiv.org/abs/2506.15645)
*Shuo Xing,Lanqing Guo,Hongyuan Hua,Seoyoung Lee,Peiran Li,Yufei Wang,Zhangyang Wang,Zhengzhong Tu*

Main category: cs.CV

TL;DR: 研究发现，多模态大语言模型（MLLMs）的性能可能因输入图像质量的改变而提升，甚至与人类感知的视觉质量相反。作者提出了一种轻量级适配模块VQ-TTT，通过动态调整图像输入，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 探索视觉输入质量对MLLMs性能的影响，揭示模型对图像质量的偏好与人类感知的差异。

Method: 通过控制图像质量退化与风格变化，研究MLLMs的反应；提出VQ-TTT模块，动态调整输入图像以适应模型偏好。

Result: 发现视觉质量悖论：图像偏离人类感知质量时，模型性能可能提升；VQ-TTT显著提高了所有测试数据集上的平均准确率。
...
### [112] [Dual-Stage Value-Guided Inference with Margin-Based Reward Adjustment for Fast and Faithful VLM Captioning](https://arxiv.org/abs/2506.15649)
*Ankan Deria,Adinath Madhavrao Dukre,Feilong Tang,Sara Atito,Sudipta Roy,Muhammad Awais,Muhammad Haris Khan,Imran Razzak*

Main category: cs.CV

TL;DR: ViMaR是一种两阶段推理框架，结合时间差分价值模型和边际感知奖励调整，提升视觉语言模型的效率和输出质量。
...
Method: 两阶段框架：首阶段选择最高价值候选描述，次阶段选择性优化弱视觉基础部分，并引入边际惩罚。
...
### [113] [UniRelight: Learning Joint Decomposition and Synthesis for Video Relighting](https://arxiv.org/abs/2506.15673)
...
Method: 联合估计反照率并合成重光照输出，利用视频扩散模型生成能力，增强场景理解和光照效果。

Result: 模型在合成和真实视频数据上训练，表现出强泛化能力，视觉保真度和时间一致性优于先前方法。
...
### [114] [Sekai: A Video Dataset towards World Exploration](https://arxiv.org/abs/2506.15675)
...
### [115] [Evolutionary Caching to Accelerate Your Off-the-Shelf Diffusion Model](https://arxiv.org/abs/2506.15682)
